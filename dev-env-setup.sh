...
echo -e "${YELLOW}Updating and upgrading the distro...${NC}"
...
if [ "$current_email" != "$email" ]; then
    echo -e "${YELLOW}Configuring Git email...${NC}"
    git config --global user.email "$email"
else
    echo -e "${GREEN}Git email is already set to $current_email. Skipping.${NC}"
fi

if [ "$current_name" != "$name" ]; then
    echo -e "${YELLOW}Configuring Git name...${NC}"
    git config --global user.name "$name"
else
    echo -e "${GREEN}Git name is already set to $current_name. Skipping.${NC}"
fi
...
if ! command -v zsh &> /dev/null; then
    ...
    echo -e "${GREEN}Zsh is already installed. Skipping.${NC}"
fi
...
if [ ! -d "$HOME/.oh-my-zsh" ]; then
    ...
    echo -e "${GREEN}Oh My Zsh is already installed. Skipping.${NC}"
fi
...
if ! command -v tldr &> /dev/null; then
    ...
    echo -e "${GREEN}tldr is already installed. Skipping.${NC}"
fi
